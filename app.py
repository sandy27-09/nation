...
def get_data_deputies():
     return pd.read_csv('C:/nation/data/df_dep.csv')
@st.cache
def get_data_political_parties():
     return pd.read_csv('C:/nation/data/df_polpar.csv')
...
st.write(df_dep)
st.write(df_pol_par)
#making the page interactive by adding filter
...
row2_spacer1, row2_1, row2_spacer2, row2_2, row2_spacer3 = st.columns((0.2, 1, .2, 1, .2))
with row2_1, _lock:
